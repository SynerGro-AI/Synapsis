...
const allLessons = [
  ...
  lesson(
    337,
    "Color Picker — Presets in a List",
    "Mixing a color by hand is fine once; a real gadget stores its favorites and lets you flip through them. Keep the presets in a list of lists — colors = [[100, 0, 0], [0, 100, 0], [0, 0, 100], [100, 100, 0]] is red, green, blue, yellow — where colors[index] is one [r, g, b] triple and colors[index][0] is that triple's red. This is the state machine from the dimmer, but the state now selects a whole color: each button press bumps index, wraps at the end, and pushes all three channels from the chosen preset. One list, one index, four colors on tap.",
    "Reuse your RGB LED (R→GPIO17, G→GPIO27, B→GPIO22, COM→Pi GND) and add a button on GPIO4. The channels are set up; store four [r, g, b] presets in a list and step through them with the button.",
    ["pi", "rgbled", "resistor", "pushbutton"],
    "import RPi.GPIO as GPIO\nimport time\n\nGPIO.setmode(GPIO.BCM)\nred_pin = 17\ngreen_pin = 27\nblue_pin = 22\nGPIO.setup(red_pin, GPIO.OUT)\nGPIO.setup(green_pin, GPIO.OUT)\nGPIO.setup(blue_pin, GPIO.OUT)\nGPIO.setup(4, GPIO.IN, pull_up_down=GPIO.PUD_UP)\n\nred = GPIO.PWM(red_pin, 1000)\ngreen = GPIO.PWM(green_pin, 1000)\nblue = GPIO.PWM(blue_pin, 1000)\nred.start(0)\ngreen.start(0)\nblue.start(0)\n\n# Store four [r, g, b] presets and step through them on each press.\n",
    [
      "colors = [[100, 0, 0], [0, 100, 0], [0, 0, 100], [100, 100, 0]]",
      "index = 0",
      "red.ChangeDutyCycle(colors[index][0])",
      "green.ChangeDutyCycle(colors[index][1])",
      "blue.ChangeDutyCycle(colors[index][2])",
      "while True:",
      "if GPIO.input(4) == GPIO.LOW:",
      "index = index + 1",
      "if index >= len(colors):",
    ...
    { initial: "Color picker: red", status: "Waiting for your Python..." },
    "rgbled",
  ),
  lesson(
    338,
    "Capstone — Smart Gate Panel",
    "Bring it all together into one device. A smart gate has a servo that swings the barrier and an RGB status light — red when closed, green when open — and a button toggles the whole thing. Wrap the panel's whole state in one function: set_gate(angle, r, g, b) drives the servo to angle (duty = angle / 18 + 2, the mapping from your servo lesson) and pushes all three color channels in one call. Keep the open/closed state in a variable, flip it on each press, and call set_gate for the matching look. Functions, lists, PWM, a servo, a button, and a state machine — every piece you've built, working as one machine.",
    "Wire a servo (PWM on GPIO18), an RGB LED (R→GPIO17, G→GPIO27, B→GPIO22, COM→Pi GND), and a button on GPIO4. Everything is set up; write set_gate(angle, r, g, b), start closed (red, 0°), and toggle open (green, 90°) on each press. Click the button to operate the gate.",
    ["pi", "servo", "rgbled", "resistor", "pushbutton"],
    "import RPi.GPIO as GPIO\nimport time\n\nGPIO.setmode(GPIO.BCM)\nGPIO.setup(18, GPIO.OUT)\nGPIO.setup(17, GPIO.OUT)\nGPIO.setup(27, GPIO.OUT)\nGPIO.setup(22, GPIO.OUT)\nGPIO.setup(4, GPIO.IN, pull_up_down=GPIO.PUD_UP)\n\ngate = GPIO.PWM(18, 50)\ngate.start(0)\nred = GPIO.PWM(17, 1000)\ngreen = GPIO.PWM(27, 1000)\nblue = GPIO.PWM(22, 1000)\nred.start(0)\ngreen.start(0)\nblue.start(0)\n\n# One function sets the whole panel: gate angle + status color.\n# Start closed (red, 0 deg); each press toggles open (green, 90 deg).\n",
    [
      "def set_gate(angle, r, g, b):",
      "duty = angle / 18 + 2",
      "gate.ChangeDutyCycle(duty)",
    ...
    { initial: "Gate: closed", status: "Waiting for your Python..." },
    "servo",
  ),
];
